...
import {IBaseAutomation} from "../../../src/interfaces/IBaseAutomation.sol";
...
contract BondAutomationTest is BaseTest {
    ...
    function setUp() public override {
        ...
        DEFAULT_ADMIN_ROLE = bondAutomation.DEFAULT_ADMIN_ROLE();
        AUTOMATION_ROLE = bondAutomation.AUTOMATION_ROLE();
    }
    ...
    function test_Constructor_GrantsAutomationRoleToAdmin() public view {
        assertTrue(bondAutomation.hasRole(AUTOMATION_ROLE, ADMIN));
    }

    function test_Constructor_RevertsWhenFunctionsConsumerIsZeroAddress() public {
        vm.expectRevert(IBondAutomation.BondAutomation__ZeroAddress.selector);
    ...
    function test_SetChainlinkForwarder_RevertsWhenCalledTwice() public {
        vm.startPrank(ADMIN);
        bondAutomation.setChainlinkForwarder(CHAINLINK_FORWARDER);
        vm.expectRevert(IBaseAutomation.BaseAutomation__ChainlinkForwarderAddressAlreadySet.selector);
        bondAutomation.setChainlinkForwarder(CHAINLINK_FORWARDER);
        vm.stopPrank();
    }
    ...
    function test_SetUpkeepId_RevertsWhenCalledTwice() public {
        vm.startPrank(ADMIN);
        bondAutomation.setUpkeepId(UPKEEP_ID);
        vm.expectRevert(IBaseAutomation.BaseAutomation__ChainlinkUpkeepIdAlreadySet.selector);
        bondAutomation.setUpkeepId(UPKEEP_ID);
        vm.stopPrank();
    }
    ...
    function test_PerformUpkeep_RevertsWhenUpkeepNotNeeded() public {
        ...
        vm.expectRevert(IBaseAutomation.BaseAutomation__UpkeepNotNeeded.selector);
        vm.prank(ADMIN);
    ...
    function test_PerformUpkeep_WithinGrace_RevertsForRandomCaller() public {
        ...
        vm.prank(ADMIN);
        vm.expectRevert(IBaseAutomation.BaseAutomation__OnlyChainlinkAutomation.selector);
        bondAutomation.performUpkeep("");
    }
    ...
    function test_PerformUpkeep_AfterGrace_AdminCanExecuteWithoutForwarderSet() public {
        ...
        bytes32 manualEventSig = keccak256("ManualUpkeepExecuted(address,uint256)");
        for(uint256 i = 0; i < logs.length; i++) {
            if (logs[i].topics[0] == manualEventSig) {
                // Found the ManualUpkeepExecuted event – verify indexed params
                address executor = address(uint160(uint256(logs[i].topics[1])));
                uint256 timestamp = uint256(logs[i].topics[2]);
                assertEq(executor, ADMIN);
                assertEq(timestamp, expectedTimestamp);
                return; // Test passes if we find the event with correct params
            }
        }
